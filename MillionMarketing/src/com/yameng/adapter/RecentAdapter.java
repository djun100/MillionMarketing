...
public class RecentAdapter extends BaseAdapter {
	...
	public RecentAdapter(Context context, LinkedList<RecentItem> data,
		...
		mMessageDB = MyApplication.getInstance().getMessageDB();
		mRecentDB = MyApplication.getInstance().getRecentDB();
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater
					.inflate(R.layout.way_recent_listview_item, null);
		}
...
//		headIV.setImageResource(MyApplication.heads[item.getHeadImg()]);
	...
	private CharSequence convertNormalStringToSpannableString(String message) {
		...
		while (localMatcher.find()) {
			...
			if (m - k < 8) {
				if (MyApplication.getInstance().getFaceMap()
						.containsKey(str2)) {
