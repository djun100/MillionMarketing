...
public class HttpRequestUtil {
	...
	public static boolean uploadFiles(String path, Map<String, String> params,
			ArrayList<FormFile> files) throws Exception {
		...
		if (files != null && files.size() != 0) {
			for (FormFile uploadFile : files) {// 得到文件类型数据的总长度
				...
				fileExplain.append("\r\n");
				System.out.println("fileExplain："+fileExplain);
		...
		System.out.println("构造socket");
		Socket socket = new Socket(InetAddress.getByName(url.getHost()), port);
		System.out.println("构造socket完成");
		...
		System.out.println("输出返回reader："+reader.readLine());
