...
public class PushMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
			...
			try {
				Message msgItem = MyApplication.getInstance().getGson()
	...
	private void parseMessage(Message msg) {
		Gson gson = MyApplication.getInstance().getGson();
		...
		if (!TextUtils.isEmpty(tag)) {// 如果是带有tag的消息
			if (userId.equals(MyApplication.getInstance().getSpUtil()
					.getUserId()))
				return;
			User u = new User(userId, msg.getChannel_id(), msg.getNick(),
					headId, 0);
			MyApplication.getInstance().getUserDB().addUser(u);// 存入或更新好友
			...
			if (MyApplication.getInstance().getSpUtil().getMsgSound())// 如果用户开启播放声音
			...
			if (ehList.size() > 0) {// 有监听的时候，传递下去
				...
				MyApplication.getInstance().getMessageDB()
						.saveMsg(userId, item);
...
//通知栏图标
		...
		PendingIntent contentIntent = PendingIntent.getActivity(application, 0,
				intent, 0);
		notification.setLatestEventInfo(MyApplication.getInstance(),
...
//				new JSONObject(content).getJSONObject("response_params");
			...
			Log.d(TAG, "将appid channelid userid 存入SP");
			SharePreferenceUtil util = MyApplication.getInstance()
					.getSpUtil();
			util.setAppId(appid);
